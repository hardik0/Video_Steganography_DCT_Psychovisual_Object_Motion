...
def extract_message(video_path, locations_file, repetition_factor=5, use_gpu=True):
    """Extract message from video using DCT-based steganography with GPU acceleration"""
    ...
    # Group locations by frame for batch processing
    frame_to_locations = {}
    for frame_num, x, y in embedding_locations:
        if frame_num not in frame_to_locations:
            frame_to_locations[frame_num] = []
        frame_to_locations[frame_num].append((x, y))
    
    # Process frames
    for frame_num in sorted(frame_to_locations.keys()):
        if frame_num > len(frames):
            print(f"Warning: Frame number {frame_num} exceeds video length")
            extraction_errors += len(frame_to_locations[frame_num])
            continue
            
        print(f"Processing frame {frame_num} with {len(frame_to_locations[frame_num])} locations")
        ...
        # Extract blocks for batch processing
        blocks = []
        for x, y in frame_to_locations[frame_num]:
            block = y_channel[y:y+8, x:x+8].astype(np.float32)
            blocks.append(block)
        
        # Batch DCT processing using GPU if available
        dct_start = time.time()
        if gpu_available and blocks:
            dct_blocks = parallel_dct_batch(blocks, use_gpu=True)
        else:
            dct_blocks = [block_dct(block, use_gpu=False) for block in blocks]
        dct_time += time.time() - dct_start
        
        # Extract bits from each block
        extract_start = time.time()
        for i, ((x, y), dct_block) in enumerate(zip(frame_to_locations[frame_num], dct_blocks)):
    ...
    print(f"  DCT operations: {dct_time:.2f} seconds ({dct_time/total_time*100:.1f}%)")
    print(f"  Extraction operations: {extract_time:.2f} seconds ({extract_time/total_time*100:.1f}%)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Extract message from video using GPU-accelerated DCT-based steganography')
    ...
    message = extract_message(args.input_video, args.locations_file, args.repetition, not args.no_gpu)

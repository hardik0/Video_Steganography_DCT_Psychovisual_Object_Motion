...
def extract_bits(coeffs):
    """Extract 3 bits from 6 coefficients"""
    bits = []
    
    # Extract first bit from coefficients 4 and 5
    # If coeff[4] > coeff[5], the bit is 1, otherwise 0
    bits.append(1 if coeffs[4] > coeffs[5] else 0)
    
    # Extract second bit from coefficients 6 and 7
    # If coeff[6] > coeff[7], the bit is 1, otherwise 0
    bits.append(1 if coeffs[6] > coeffs[7] else 0)
    
    # Extract third bit from coefficients 10 and 11
    # If coeff[10] > coeff[11], the bit is 1, otherwise 0
    bits.append(1 if coeffs[10] > coeffs[11] else 0)
    
    return bits

def extract_message(video_path, locations_file):
    ...
    # Apply error correction - repetition code (5x)
    corrected_bits = []
    if len(extracted_bits) >= 5:
        # Process in groups of 5 bits
        for i in range(0, len(extracted_bits), 5):
            if i + 5 <= len(extracted_bits):
                group = extracted_bits[i:i+5]
                # Majority vote (3 out of 5 bits)
                bit = 1 if sum(group) >= 3 else 0
    ...
    try:
        ...
        if len(original_binary) == len(corrected_bits_str):
            ...
            for i, (a, b) in enumerate(zip(original_binary, corrected_bits_str)):
                if a != b and i < 40:  # Show first 5 differences (limited to first 40 bits)
...
if __name__ == "__main__":
    ...
    message = extract_message(args.input_video, args.locations_file)
